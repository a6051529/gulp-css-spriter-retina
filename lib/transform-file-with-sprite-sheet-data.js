...
function transformFileWithSpriteSheetData(vinylFile, coordinateMap, pathToSpriteSheetFromCSS,  /*optional*/includeMode, /*optional*/isSilent, /*optional*/outputIndent, scale, bgImageSize) {
	...
	if(resultantFile) {
		...
		styles = mapOverStylesAndTransformBackgroundImageDeclarations(styles, includeMode, function(declaration) {
			...
			declaration.value = spriterUtil.matchBackgroundImages(declaration.value, function(imagePath) {
				...
				if(coords) {
					coordList.push("-" + (coords.x * scale) + "px -" + (coords.y * scale) + "px");
